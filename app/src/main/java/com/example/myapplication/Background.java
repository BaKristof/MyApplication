...
public class Background {
    ...
    private void convertcomptoseab() {
        ...
        Random r = new Random();
        int i1 = r.nextInt(completback.length);
        int i2 = r.nextInt(completback.length);
        for (int i = 0; i < seableback.length; i++) {
            for (int j = 0; j < seableback[i].length; j++) {
                seableback[i][j] = convertfromint(completback[i][j], i, j, tileshightandwidth);
            }
        }
    }
